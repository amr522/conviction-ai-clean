...
def build_daily_master(
    stocks_daily, options_daily, fred_df=None, vix_df=None, dxy_df=None, news_df=None
):
    ...
    if fred_df is not None:
        fred_renamed = fred_df.rename(
            {col: f"fred_{col}" for col in fred_df.columns if col != "date"}
        )
        master = master.join(fred_renamed, on="date", how="left")

    if vix_df is not None:
        vix_renamed = vix_df.rename(
            {col: f"vix_{col}" for col in vix_df.columns if col != "date"}
        )
        master = master.join(vix_renamed, on="date", how="left")

    if dxy_df is not None:
        dxy_renamed = dxy_df.rename(
            {col: f"dxy_{col}" for col in dxy_df.columns if col != "date"}
        )
...
class TestBuildDailyMaster:
    ...
    def test_missing_macro_data_results_in_nulls(self):
        ...
        fred_df = pl.DataFrame(
            {"date": [pd.to_datetime("2025-01-01")], "fed_funds_rate": [5.25]}
        )
    ...
    def test_joined_master_has_correct_row_count_and_columns(self):
        ...
        result = build_daily_master(
            stocks_daily, options_daily, fred_df=fred_df, vix_df=vix_df
        )
    ...
    def test_macro_columns_correctly_prefixed(self):
        ...
        vix_df = pl.DataFrame({"date": [pd.to_datetime("2025-01-01")], "close": [18.5]})

        dxy_df = pl.DataFrame({"date": [pd.to_datetime("2025-01-01")], "rate": [103.5]})
