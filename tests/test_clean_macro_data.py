...
class TestCleanMacroData:
    ...
    def test_load_data_source_force_raw(self, tmp_path):
        ...
        result = load_data_source("TEST", str(raw_path), str(parquet_path), use_raw=True)
    ...
    def test_load_data_source_auto_fallback(self, tmp_path):
        ...
        result = load_data_source("TEST", str(raw_path), str(parquet_path), use_raw=False)
    ...
    def test_load_data_source_json(self, tmp_path):
        ...
        result = load_data_source("TEST", str(raw_path), str(parquet_path), use_raw=True, is_json=True)
        assert len(result) == 2
        assert pd.api.types.is_datetime64_any_dtype(result["date"])
        assert result["value"].tolist() == [1.0, 2.0]
    ...
    def test_load_data_source_returns_correct_dataframe(self, tmp_path):
        ...
        # Test processed path
        result_proc = load_data_source("FRED", str(raw_path), str(parquet_path), use_raw=False)
        assert result_proc["fed_funds_rate"].tolist() == [5.20, 5.25]
        
        # Test raw path
        result_raw = load_data_source("FRED", str(raw_path), str(parquet_path), use_raw=True)
    ...
    def test_detect_backshift_correctly_identifies_newer_raw(self):
        ...
        raw_df = pd.DataFrame({"date": pd.to_datetime(["2025-01-01", "2025-01-02"])})
        assert detect_backshift("TEST", proc_df, raw_df) is True  # Different dates trigger fallback
