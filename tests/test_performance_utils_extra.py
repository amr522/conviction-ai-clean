...
def make_df():
    ...
    return pl.DataFrame({
        "opt30_volume": [10, 20, 30, 40, 50],
        "opt30_net_gamma": [1, 2, 3, 4, 5]
    })
...
def test_optimize_signal_generation():
    ...
    assert out.select("rolling_vol_mean").item(2, 0) == pytest.approx((10+20+30)/3)
    
    # Check all expected columns are present
    expected_cols = ["rolling_vol_mean", "rolling_vol_std", "rolling_gamma_mean", "rolling_gamma_std"]
