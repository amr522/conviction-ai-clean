...
class TestDataSourcePinning(unittest.TestCase):
    ...
    def test_get_pinned_dataset_no_source(self):
        """Test behavior when no pinned dataset source is available"""
        with patch.dict(os.environ, {}, clear=True):
            result = get_pinned_dataset()
            self.assertIsNone(result)
    
    def test_get_pinned_dataset_file_read_error(self):
    ...
    def test_validate_s3_uri_format_invalid(self):
        ...
        result = validate_s3_uri(self.invalid_s3_uri)
...
class TestSageMakerMocking(unittest.TestCase):
    ...
    def test_launch_aapl_hpo_dry_run(self, mock_boto3, mock_sagemaker):
        ...
        fit_call_args = mock_tuner.fit.call_args
        training_data = fit_call_args[0][0]['training']
        self.assertEqual(training_data, pinned_uri)
    ...
    def test_launch_full_universe_hpo_dry_run(self, mock_boto3, mock_sagemaker):
        ...
        with patch.dict(os.environ, {'PINNED_DATA_S3': pinned_uri}):
            result = launch_full_universe_hpo()
    ...
    def test_fallback_to_default_data_source(self, mock_boto3, mock_sagemaker):
        ...
        fit_call_args = mock_tuner.fit.call_args
        training_data = fit_call_args[0][0]['training']
        self.assertTrue(training_data.startswith('s3://hpo-bucket-773934887314/data/'))
