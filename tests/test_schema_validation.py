...
from validate_schemas import (SchemaValidationError, _types_match,
                              compare_schemas, load_expected_schema,
...
def test_load_expected_schema():
    ...
    with tempfile.NamedTemporaryFile(mode="w", suffix=".json", delete=False) as f:
        schema_spec = {
            "test_dataset": {"col1": "float64", "col2": "string", "col3": "bool"}
        }
...
def test_load_expected_schema_missing_dataset():
    ...
    try:
        with pytest.raises(
            ValueError, match="Dataset type 'missing_dataset' not found"
        ):
...
def test_compare_schemas():
    ...
    actual = {"col1": "double", "col2": "string", "col3": "int64", "extra_col": "bool"}

    expected = {"col1": "float64", "col2": "string", "missing_col": "uint64"}
...
def test_validate_parquet_schema_extra_column():
    ...
    # Create temporary spec file
    with tempfile.NamedTemporaryFile(mode="w", suffix=".json", delete=False) as f:
        schema_spec = {"test_dataset": {"col1": "float64"}}  # extra_col is not expected
        json.dump(schema_spec, f)
        spec_file = f.name
...
def test_validate_parquet_schema_type_mismatch():
    ...
    # Create temporary spec file
    with tempfile.NamedTemporaryFile(mode="w", suffix=".json", delete=False) as f:
        schema_spec = {"test_dataset": {"col1": "float64"}}  # Actual is string
        json.dump(schema_spec, f)
        spec_file = f.name
    ...
    try:
        with patch("validate_schemas.pq.read_table", return_value=mock_table):
            ...
            assert "Type mismatches: col1: expected float64, got string" in str(
                exc_info.value
            )
    finally:
        os.unlink(spec_file)
...
def test_validate_parquet_schema_success():
    ...
    # Create temporary spec file
    with tempfile.NamedTemporaryFile(mode="w", suffix=".json", delete=False) as f:
        schema_spec = {"test_dataset": {"col1": "float64", "col2": "string"}}
        json.dump(schema_spec, f)
        spec_file = f.name
