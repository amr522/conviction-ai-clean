...
from calculate_features import calculate_rolling_features, calculate_intraday_features, calculate_cross_sectional_features
...
class TestBenchmarks:
    ...
    def test_bench_intraday_returns(self, benchmark):
        ...
        df = pl.DataFrame({
            ...
            "opt30_mid_price": [100.0 + i*0.01 + (i%100-50)*0.1 for i in range(len(timestamps))]
        })
    ...
    def test_bench_vol_zscore(self, benchmark):
        ...
        df = pl.DataFrame({
            ...
            "optd_volume": [1000 + i*10 + (i%1000-500)*5 for i in range(total_rows)],
            "optd_iv30": [0.25 + (i%100)*0.001 for i in range(total_rows)],
            "stockd_return_1d": [0.01 + (i%200-100)*0.0001 for i in range(total_rows)]
        })
    ...
    def test_bench_full_pipeline(self, benchmark):
        ...
        # Daily master data
        dates = pl.date_range(date(2025, 1, 1), date(2025, 3, 31), "1d", eager=True)[:n_days]
        repeated_dates = []
        for _ in range(n_tickers):
        ...
        daily_df = pl.DataFrame({
            "date": repeated_dates,
            "ticker": [f"STOCK_{i%n_tickers:02d}" for i in range(n_days * n_tickers)],
            "fred_fed_funds_rate": [3.0 + (i%100)*0.01 for i in range(n_days * n_tickers)],
            "vix_index": [20.0 + (i%50)*0.5 for i in range(n_days * n_tickers)],
            "news_count": [5 + i%15 for i in range(n_days * n_tickers)],
            "optd_iv30": [0.25 + (i%200)*0.001 for i in range(n_days * n_tickers)],
            "optd_volume": [1000 + i*50 for i in range(n_days * n_tickers)],
            "stockd_return_1d": [0.01 + (i%400-200)*0.0001 for i in range(n_days * n_tickers)],
