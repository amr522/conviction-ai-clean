...
def test_run_full_pipeline_integration(sample_master_data):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        env = os.environ.copy()
        env.update({"WINDOW_DAYS": "3", "USE_GPU": "false", "N_JOBS": "1"})
        ...
        try:
            ...
            cmd = [
                ...
                "--output-path",
                "datasets/features_2025-01-16.parquet",
                "--date",
                "2025-01-16",
                "--window-days",
                "3",
            ...
            assert (
                result.returncode == 0
            ), f"Feature calculation failed: {result.stderr}"
...
def test_environment_variable_integration():
    ...
    env_vars = {"WINDOW_DAYS": "20", "USE_GPU": "true", "N_JOBS": "4"}
