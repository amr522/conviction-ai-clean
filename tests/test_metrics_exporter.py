...
def test_empty_directories_metrics(metrics_server, test_data_dir):
    ...
    for line in file_count_lines:
        if (
            'directory="datasets"' in line
            or 'directory="master"' in line
            or 'directory="staged"' in line
        ):
...
def test_missing_output_simulation(metrics_server, test_data_dir):
    ...
    lines = content.split("\n")
    status_line = [
        line for line in lines if line.startswith("vol_pipeline_last_run_status ")
    ]
...
def test_fresh_data_simulation(metrics_server, test_data_dir):
    ...
    lines = content.split("\n")
    age_line = [
        line for line in lines if line.startswith("vol_pipeline_data_age_hours ")
    ]
...
def test_file_count_with_data(metrics_server, test_data_dir):
    ...
    datasets_line = [
        line
        for line in lines
        if 'vol_pipeline_files_total{directory="datasets"}' in line
    ]
    ...
    master_line = [
        line for line in lines if 'vol_pipeline_files_total{directory="master"}' in line
    ]
...
def test_size_metrics_with_data(metrics_server, test_data_dir):
    ...
    lines = content.split("\n")
    size_line = [
        line for line in lines if 'vol_pipeline_size_mb{directory="datasets"}' in line
    ]
...
def test_runtime_metric_default(metrics_server):
    ...
    lines = content.split("\n")
    runtime_line = [
        line for line in lines if line.startswith("vol_pipeline_runtime_seconds ")
    ]
