...
@pytest.mark.parametrize("f1,f2", combinations(load_feature_list("docs/features_list.md"), 2))
def test_low_correlation(feature_df, f1, f2):
    ...
    if corr_result.is_null().any():
        pytest.skip(f"Cannot compute correlation between {f1} and {f2} (constant features)")
...
def test_feature_variance():
    ...
    assert not low_variance_features, f"Features with low variance: {low_variance_features}"