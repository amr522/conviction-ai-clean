...
), patch("aws_xray_sdk.ext.fastapi.XRayMiddleware"), patch(
    "app.metrics.update_model_info"
):
...
class TestAPISecurity:
    def setup_method(self):
        ...
        self.valid_token = generate_test_token(
            "test_user", ["predict", "batch", "admin"]
        )
        self.predict_only_token = create_access_token(
            "user_123", "predict_user", ["predict"]
        )
    ...
    def test_batch_predict_requires_batch_permission(self):
        ...
        response = self.client.post(
            "/predict/batch", json=request_data, headers=headers
        )
...
def test_security_imports():
    ...
    try:
        ...
        from app.metrics import get_metrics, track_predictions
