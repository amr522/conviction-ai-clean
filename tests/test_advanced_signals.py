...
class TestAdvancedSignals:
    ...
    def test_pcr_percentile_calculation(self):
        ...
        df = pl.DataFrame({
            ...
            'optd_put_call_ratio': [0.5, 1.0, 1.5, 2.0, 0.6, 1.1, 1.6, 2.1, 
                                   0.7, 1.2, 1.7, 2.2, 0.8, 1.3, 1.8, 2.3,
                                   0.9, 1.4, 1.9, 2.4],
        ...
        df = df.with_columns([
            pl.col('optd_put_call_ratio').rank(method='average').truediv(pl.len()).alias('optd_pcr_pctile')
        ])
        ...
        assert df['optd_pcr_extreme'].dtype == pl.Boolean, "PCR extreme should be boolean"
    ...
    def test_vrp_spike_detection(self):
        ...
        df = df.with_columns([
            pl.col('optd_vrp_30d').rolling_quantile(0.9, window_size=20).alias('vrp_90pct'),
            (pl.col('optd_vrp_30d') > pl.col('optd_vrp_30d').rolling_quantile(0.9, window_size=20)).alias('optd_vrp_spike')
        ])
    ...
    def test_flow_divergence_calculation(self):
        ...
        df = pl.DataFrame({
            ...
            'timestamp': pd.date_range('2025-01-01 09:30', periods=10, freq='30min')
        })
        
        # Calculate flow divergence
        df = df.with_columns([
            ((pl.col('opt30_call_volume') - pl.col('opt30_put_volume')) / 
             (pl.col('opt30_call_volume') + pl.col('opt30_put_volume'))).alias('opt30_flow_divergence')
        ])
        
        # Test calculation
        expected_divergence = (100 - 50) / (100 + 50)  # First row
        actual_divergence = df['opt30_flow_divergence'][0]
        assert abs(actual_divergence - expected_divergence) < 1e-6, "Flow divergence calculation incorrect"
        
        # Test range
        assert df['opt30_flow_divergence'].min() >= -1.0, "Flow divergence should be >= -1"
        assert df['opt30_flow_divergence'].max() <= 1.0, "Flow divergence should be <= 1"
    ...
    def test_gamma_squeeze_detection(self):
        ...
        df = pl.DataFrame({
            ...
            'timestamp': pd.date_range('2025-01-01 09:30', periods=25, freq='30min')
        })
        
        # Calculate net gamma and rolling statistics
        df = df.with_columns([
            (pl.col('opt30_gamma') * pl.col('opt30_open_interest')).alias('opt30_net_gamma')
        ]).with_columns([
            pl.col('opt30_net_gamma').rolling_mean(5).alias('opt30_gamma_mean_5'),
            pl.col('opt30_net_gamma').rolling_std(5).alias('opt30_gamma_std_5')
        ]).with_columns([
            (pl.col('opt30_net_gamma') > 
             (pl.col('opt30_gamma_mean_5') + 2.0 * pl.col('opt30_gamma_std_5'))).alias('opt30_gamma_squeeze')
        ])
        
        # Test gamma squeeze detection
        assert df['opt30_gamma_squeeze'].dtype == pl.Boolean, "Gamma squeeze should be boolean"
    ...
    def test_signal_data_types(self):
        ...
        df = df.with_columns([
            pl.col('optd_put_call_ratio').rank().truediv(pl.len()).alias('optd_pcr_pctile'),
        ...
        # Test data types
        assert df['optd_pcr_pctile'].dtype == pl.Float64, "PCR percentile should be Float64"
        assert df['optd_pcr_extreme'].dtype == pl.Boolean, "PCR extreme should be Boolean"
        assert df['optd_vrp_spike'].dtype == pl.Boolean, "VRP spike should be Boolean"
        assert df['opt30_flow_divergence'].dtype == pl.Float64, "Flow divergence should be Float64"
    ...
    def test_edge_cases(self):
        ...
        df_zero = df_zero.with_columns([
            ((pl.col('opt30_call_volume') - pl.col('opt30_put_volume')) / 
             (pl.col('opt30_call_volume') + pl.col('opt30_put_volume') + 1e-8)).alias('opt30_flow_divergence')
        ])
        
        assert not df_zero['opt30_flow_divergence'].is_null().any(), "Should handle zero volume case"
        ...
        assert df_single['optd_pcr_pctile'][0] == 1.0, "Single value should have percentile 1.0"
