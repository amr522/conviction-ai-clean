...
from src.utils.profiling import enable_profiling, save_profile_report, clear_profile_results
    ...
    if result['status'] == 'success':
        ...
        if args.profile:
            ...
            if PROFILE_RESULTS:
                ...
                sorted_results = sorted(PROFILE_RESULTS, key=lambda x: x['duration_seconds'], reverse=True)
                print("  Top 3 slowest functions:")
                for i, result in enumerate(sorted_results[:3], 1):
                    print(f"    {i}. {result['function']}: {result['duration_seconds']:.3f}s")
    else:
