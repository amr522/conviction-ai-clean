...
def send_drift_notification(dt: str, drift_detected: bool, backfill_success: bool = None):
...
def auto_retrain_flow(target_date: str = None):
    ...
    if drift_detected:
        logger.warning(f"⚠️ Data drift detected for {target_date}, triggering historical backfill")
        
        backfill_success = trigger_backfill(target_date)
        
        if backfill_success:
            logger.info(f"✅ Historical backfill completed successfully")
            send_drift_notification(target_date, drift_detected=True, backfill_success=True)
            return {"status": "success", "drift_detected": True, "backfill_completed": True}
        else:
            logger.error(f"❌ Historical backfill failed")
            send_drift_notification(target_date, drift_detected=True, backfill_success=False)
            return {"status": "partial_success", "drift_detected": True, "backfill_completed": False}
    else:
        logger.info(f"✅ No data drift detected for {target_date}, skipping historical backfill")
        send_drift_notification(target_date, drift_detected=False)
        return {"status": "success", "drift_detected": False, "backfill_completed": False}
