...
def run(date: str, dry_run: bool = False, use_raw_macro: bool = False, 
    ...
    try:
        ...
        vix_parquet_path = os.path.join(project_root, "data/Parquet_data/vix_data.parquet")
        dxy_parquet_path = os.path.join(project_root, "data/Parquet_data/dxy.parquet")
        news_parquet_path = os.path.join(project_root, "data/Parquet_data/news_data.parquet")
        ...
        try:
            ...
            missing_cols = [col for col in required_stock_cols if col not in stocks_data.columns]
            if missing_cols:
            ...
            null_cols = [
                ...
                if stocks_data[col].null_count() > 0
            ]
            if null_cols:
        ...
        try:
            ...
            missing_cols = [col for col in required_option_cols if col not in options_data.columns]
            if missing_cols:
                raise ValueError(f"Missing required columns in options data: {missing_cols}")
            
            # Check for null values in critical columns
            null_cols = [
                col for col in required_option_cols 
        ...
        fred_df = load_data_source("FRED", raw_fred_csv, fred_parquet_path, use_raw_macro)
        vix_df = load_data_source("VIX", raw_vix_json, vix_parquet_path, use_raw_macro, is_json=True)
        dxy_df = load_data_source("DXY", raw_dxy_csv, dxy_parquet_path, use_raw_macro)
        news_df = load_data_source("News", raw_news_dir, news_parquet_path, use_raw_macro)
        ...
        if not fred_df.empty:
            ...
            fred_pl = fred_pl.rename({col: f"fred_{col}" for col in fred_pl.columns if col != "date"})
            macro_dfs['fred'] = fred_pl
            print(f"FRED data: {fred_pl.shape}")
        
        if not vix_df.empty:
            vix_pl = pl.from_pandas(vix_df)
            vix_pl = vix_pl.rename({col: f"vix_{col}" for col in vix_pl.columns if col != "date"})
            macro_dfs['vix'] = vix_pl
            print(f"VIX data: {vix_pl.shape}")
        
        if not dxy_df.empty:
            dxy_pl = pl.from_pandas(dxy_df)
            dxy_pl = dxy_pl.rename({col: f"dxy_{col}" for col in dxy_pl.columns if col != "date"})
        ...
        if not news_df.empty:
            ...
            news_agg = news_pl.group_by("date").agg([
                ...
                pl.col("sentiment").mean().alias("news_avg_sentiment") if "sentiment" in news_pl.columns else pl.lit(0.0).alias("news_avg_sentiment")
            ])
        ...
        try:
            ...
            stock_cols = ["date", "ticker"] + [
                col for col in stocks_data.columns 
                if col.startswith("stockd_")
            ]
            option_cols = ["date", "ticker"] + [
                col for col in options_data.columns 
                if col.startswith("optd_")
            ]
            ...
            feature_dtypes = {
                # stocks daily
                'stockd_close': 'float64', 'stockd_volume': 'UInt64',
                'stockd_return_1d': 'float64', 'stockd_return_1d_lag1': 'float64',
                'stockd_vol_7d': 'float64', 'stockd_vol_7d_lag1': 'float64',
                'stockd_volume_pct_change': 'float64',
                'stockd_beta_spy': 'float64', 'stockd_days_to_earnings': 'Int64',
                # options daily
                'optd_strike': 'float64', 'optd_moneyness': 'float64',
                'optd_iv30': 'float64', 'optd_hv30': 'float64',
                'optd_iv30_lag1': 'float64', 'optd_hv30_lag1': 'float64',
                'optd_iv_percentile': 'float64', 'optd_iv_percentile_lag1': 'float64',
                'optd_vrp_30d': 'float64', 'optd_vrp_30d_lag1': 'float64',
                'optd_iv_skew_slope': 'float64', 'optd_vol_surprise': 'float64',
                'optd_smile_convexity': 'float64', 'optd_ivrv_spread': 'float64',
                'optd_put_call_ratio': 'float64', 'optd_chain_liquidity_lag1': 'float64',
                'optd_volume': 'UInt64',
                # options 30min
                'opt30_strike': 'float64', 'opt30_moneyness': 'float64',
                'opt30_mid_price_return': 'float64', 'opt30_bid_ask_spread': 'float64',
                'opt30_implied_volatility': 'float64', 'opt30_delta': 'float64',
                'opt30_theta': 'float64', 'opt30_volume_return': 'float64',
                'opt30_open_interest_change': 'float64', 'opt30_rolling_vol_5': 'float64',
                # macro features
                'fred_fed_funds_rate': 'float64', 'fred_unemployment_rate': 'float64',
                'fred_inflation_rate': 'float64', 'vix_index': 'float64',
                'vix_close': 'float64', 'dxy_rate': 'float64', 'dxy_close': 'float64',
                'news_count': 'UInt64', 'news_avg_sentiment': 'float64',
            }
            ...
            critical_cols = ["date", "ticker"] + required_stock_cols + required_option_cols
            null_cols = [
        ...
        if not dry_run:
            try:
                ...
                final_schema = {
                    col: dtype for col, dtype in master_dtypes.items()
                ...
                for col, dtype in final_schema.items():
                    if df[col].dtype != dtype:
                        type_mismatches.append(f"{col}: got {df[col].dtype}, expected {dtype}")
                
                if type_mismatches:
                    raise ValueError(f"Type validation failed:\n" + "\n".join(type_mismatches))
        ...
        if "data type mismatch" in str(e).lower():
            ...
            print("This usually means one of the cleaning scripts is not enforcing proper data types.")
            print("Please verify that all cleaning scripts are using correct type casting.")
        else:
