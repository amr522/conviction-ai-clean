...
class LineageTracker:
    ...
    def __init__(self, namespace: str = "conviction_ai"):
        self.client = OpenLineageClient.from_environment() if os.getenv("OPENLINEAGE_URL") else None
        self.namespace = namespace
        self.current_run = None
    
    def start_run(self, job_name: str, inputs: List[str], outputs: List[str], 
                  run_id: Optional[str] = None, sql: Optional[str] = None) -> Optional[Run]:
        """Start a lineage run."""
        if not self.client:
            return None
...
def track_lineage(job_name: str, inputs: List[str], outputs: List[str], sql: Optional[str] = None):
