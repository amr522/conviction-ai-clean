...
def configure_xray(
    service_name: str = "conviction-ai", daemon_address: str = "127.0.0.1:2000"
):
    """
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            try:
                ...
                if (
                    not os.getenv("AWS_XRAY_TRACING_DISABLED", "false").lower()
                    == "true"
                ):
                    try:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        try:
            if exc_type is not None:
                xray_recorder.put_annotation("error", True)
                xray_recorder.put_metadata(
                    "exception", {"type": exc_type.__name__, "message": str(exc_val)}
                )
...
def traced_subsegment(
    name: str, annotations: Optional[Dict] = None, metadata: Optional[Dict] = None
):
    """
