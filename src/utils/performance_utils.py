...
@profile_time
def optimize_join_performance(stocks_df: pl.LazyFrame, options_df: pl.LazyFrame, on: list = None) -> pl.DataFrame:
    """
...
@profile_time
def compute_flow_signals_optimized(df: pl.DataFrame, window: int = None) -> pl.DataFrame:
    """
    ...
    if 'opt30_type' not in df.columns:
        df = df.with_columns([
            pl.col("ticker").str.extract(r"([CP])", 1).fill_null("C").alias("opt30_type")
        ])
    
    # Compute flow aggregations
    flow_agg = (
        df
        .group_by(["underlying", "timestamp", "opt30_type"])
        .agg([pl.col("opt30_volume").sum().alias("flow_volume")])
        .pivot(index=["underlying", "timestamp"], columns="opt30_type", values="flow_volume")
        .with_columns([
            pl.col("C").fill_null(0).alias("opt30_call_flow"),
            pl.col("P").fill_null(0).alias("opt30_put_flow")
        ])
        .with_columns([
            (pl.col("opt30_call_flow") - pl.col("opt30_put_flow")).alias("opt30_flow_divergence")
        ])
        .select(["underlying", "timestamp", "opt30_call_flow", "opt30_put_flow", "opt30_flow_divergence"])
    )
...
@profile_time
def compute_gamma_signals_optimized(df: pl.DataFrame, window: int = None, gamma_squeeze_multiplier: float = None) -> pl.DataFrame:
    """
    ...
    print(f"Computing gamma signals with window={window}, multiplier={gamma_squeeze_multiplier}...")
    ...
    result = (
        ...
        .with_columns([
            ...
            (pl.col("opt30_net_gamma") > 
             (pl.col(f"opt30_gamma_mean_{window}") + gamma_squeeze_multiplier * pl.col(f"opt30_gamma_std_{window}"))
