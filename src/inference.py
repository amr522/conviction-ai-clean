...
def explain_predictions(model, X: pl.DataFrame, pushgateway_url=None):
    ...
    print(
        f"Computing SHAP explanations for {X_np.shape[1]} features on {X_np.shape[0]} samples"
    )
...
def push_shap_metrics(shap_summary: dict, url: str, job: str = "model_explain"):
    ...
    try:
        ...
        shap_gauge = Gauge(
            "shap_mean_abs", "Mean absolute SHAP value", ["feature"], registry=registry
        )
        ...
        total_gauge = Gauge(
            "shap_total_importance", "Total SHAP importance", registry=registry
        )
        max_gauge = Gauge(
            "shap_max_importance", "Maximum feature SHAP importance", registry=registry
        )
        features_gauge = Gauge(
            "shap_features_analyzed", "Number of features analyzed", registry=registry
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--feature-path", required=True, help="Path to features parquet"
    )
