...
def compare_schemas(actual: Dict[str, str], expected: Dict[str, str]) -> Tuple[List[str], List[str], List[str]]:
    """
...
def validate_parquet_schema(parquet_path: str, spec_file: str, dataset_type: str) -> None:
    """
    ...
    # Compare schemas
    missing_cols, extra_cols, type_mismatches = compare_schemas(actual_schema, expected_schema)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Validate Parquet schema against specification")
    parser.add_argument("--parquet-path", required=True, help="Path to Parquet file")
    parser.add_argument("--spec-file", default="schemas/option_parquet_schema.json", help="Schema specification file")
    parser.add_argument("--dataset-type", required=True, help="Dataset type (options_daily, options_30min, etc.)")
