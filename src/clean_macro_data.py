...
def load_data_source(name: str, raw_path: str, parquet_path: str, use_raw: bool, is_json: bool = False) -> pd.DataFrame:
...
def main():
    ...
    parser.add_argument("--use-raw-macro", action="store_true", help="Force use of raw macro data")
    ...
    # Load VIX data
    vix_df = load_data_source("VIX", args.raw_vix_json, vix_out, args.use_raw_macro, is_json=True)
    ...
    if args.use_raw_macro or not os.path.exists(news_out):
        ...
        if not news_df.empty:
            ...
            news_df = pd.read_parquet(news_out) if os.path.exists(news_out) else pd.DataFrame()
    else:
        news_df = pd.read_parquet(news_out) if os.path.exists(news_out) else pd.DataFrame()
