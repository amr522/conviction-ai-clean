...
def run(date: str, dry_run: bool = False) -> dict:
    ...
    try:
        ...
        if validation_results["daily_master"] and validation_results.get("daily_partitions"):
            ...
            if master_rows != partition_rows:
                print(f"WARNING: Daily row count mismatch - Master: {master_rows}, Partitions: {partition_rows}")
        
        # Compare intraday master with its partitions  
        if validation_results["intraday_master"] and validation_results.get("intraday_partitions"):
            master_rows = validation_results["intraday_master"]["row_count"]
            partition_rows = validation_results["intraday_partitions"]["total_rows"]
            
            if master_rows != partition_rows:
                print(f"WARNING: Intraday row count mismatch - Master: {master_rows}, Partitions: {partition_rows}")
