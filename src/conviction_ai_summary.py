...
def format_status_output(status: Dict, format_type: str = "text") -> str:
    ...
    if feat["status"] == "available":
        output.append(f"✅ Features: {feat['count']:,} records, {feat.get('columns', 0)} columns")
    else:
    ...
    for feature_file in sorted(feature_files, reverse=True)[:days]:
        ...
        try:
            ...
            runs.append({
                ...
                "has_training_data": status["training_dataset"]["status"] == "available",
    ...
    if args.recent:
        ...
        if args.format == "json":
            ...
            for run in runs:
                ...
                status_icons.append("✅" if run["validation_status"] == "available" else "❌")
                
                print(f"{run['date']}: {' '.join(status_icons)} ({run['features_count']:,} features)")
    else:
