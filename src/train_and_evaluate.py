...
def monitor_feature_importance(model, feature_names: list, top_n: int = 20, n_jobs: int = 1) -> Dict:
    """
...
def load_partitioned_data(data_dir: str, start_date: str, end_date: str) -> pd.DataFrame:
    """
    ...
    for year_dir in data_path.glob("year=*"):
        ...
        for month_dir in year_dir.glob("month=*"):
            ...
            partition_end = partition_start + pd.DateOffset(months=1) - pd.Timedelta(days=1)
...
def prepare_features_and_target(intraday_df: pd.DataFrame, daily_df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.Series]:
    """
    ...
    if 'timestamp' in intraday_df.columns and 'timestamp' in daily_df.columns:
        ...
        combined_df = pd.merge(intraday_df, daily_df, on=merge_keys, how='inner', suffixes=('_intraday', '_daily'))
    else:
    ...
    if 'close' in combined_df.columns:
        ...
        combined_df['volatility_target'] = combined_df['returns'].rolling(window=5).std().shift(-1)
    else:
        # Fallback: create synthetic target
        np.random.seed(42)
        combined_df['volatility_target'] = np.random.normal(0.02, 0.01, len(combined_df))
    ...
    logger.info(f"Prepared {len(feature_cols)} features and {len(target)} target values")
    return features, target


def train_validation_split(features: pd.DataFrame, target: pd.Series, train_ratio: float = 0.8) -> Tuple:
    """
...
def optimize_hyperparameters(X_train: pd.DataFrame, y_train: pd.Series, 
                            ...
                            n_trials: int = 50, use_gpu: bool = False, n_jobs: int = 1) -> Dict:
    """
    ...
    logger.info(f"Starting hyperparameter optimization with {n_trials} trials using {n_jobs} workers...")
...
def train_model(X_train: pd.DataFrame, y_train: pd.Series, params: Dict = None, use_gpu: bool = False, n_jobs: int = 1) -> LGBMRegressor:
    """
...
def run(start_date: str, end_date: str, model_path: str, metrics_path: str, dry_run: bool = False, tune: bool = False, n_trials: int = 50, n_jobs: int = None) -> int:
    """
    ...
    try:
        ...
        if tracking_uri:
            ...
            mlflow.set_experiment(os.getenv("MLFLOW_EXPERIMENT_NAME", "ConvictionAI-Swing"))
        ...
        intraday_df = load_partitioned_data("datasets/intraday_30m", start_date, end_date)
        ...
        if dry_run:
            ...
            logger.info(f"Would train on {len(features)} samples with {len(features.columns)} features")
            if tune:
                logger.info(f"Running 1 trial for dry-run hyperparameter optimization (GPU: {use_gpu}, Workers: {n_jobs})...")
                best_params = optimize_hyperparameters(X_train, y_train, X_val, y_val, n_trials=1, use_gpu=use_gpu, n_jobs=n_jobs)
        ...
        run_context = mlflow.start_run(run_name=f"train_{start_date}_{end_date}") if tracking_uri else None
        ...
        try:
            ...
            best_params = None
            if tune:
                logger.info(f"Starting hyperparameter optimization with {n_trials} trials (GPU: {use_gpu}, Workers: {n_jobs})")
                best_params = optimize_hyperparameters(X_train, y_train, X_val, y_val, n_trials=n_trials, use_gpu=use_gpu, n_jobs=n_jobs)
                ...
                params_file = os.path.join(metrics_path, f"optuna_best_params_{start_date}_{end_date}.json")
                logger.info(f"Saving best parameters to {params_file}")
                with open(params_file, 'w') as f:
                    json.dump(best_params, f, indent=2)
        
            # Train model with best parameters or defaults
            model = train_model(X_train, y_train, best_params, use_gpu=use_gpu, n_jobs=n_jobs)
            
            # Monitor feature importance
            feature_importance = monitor_feature_importance(model, list(features.columns), n_jobs=n_jobs)
            ...
            if tracking_uri:
                mlflow.sklearn.log_model(
                    model, 
                    "model", 
                    registered_model_name="ConvictionAI_Swing_Model"
                )
        
            # Save metrics
            metrics_file = os.path.join(metrics_path, f"metrics_{start_date}_{end_date}.json")
            full_metrics = {
                'evaluation': metrics,
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Automated model training and evaluation")
    parser.add_argument("--start-date", required=True, help="Start date (YYYY-MM-DD)")
    parser.add_argument("--end-date", required=True, help="End date (YYYY-MM-DD)")
    parser.add_argument("--model-path", default="models/latest.pkl", help="Path to save model")
    parser.add_argument("--metrics-path", default="metrics/", help="Directory to save metrics")
    parser.add_argument("--dry-run", action="store_true", help="Run without training or saving files")
    parser.add_argument("--tune", action="store_true", help="Run hyperparameter optimization with Optuna")
    parser.add_argument("--n-trials", type=int, default=50, help="Number of Optuna trials for hyperparameter optimization")
    parser.add_argument("--n-jobs", type=int, default=None, help="Number of parallel workers (defaults to os.cpu_count())")
