...
from utils.profiling import enable_profiling, save_profile_report, clear_profile_results

@xray_recorder.capture('etl_pipeline')
def run_full_pipeline(date: str, dry_run: bool = False, flow_window: int = 1, 
                     gamma_squeeze_multiplier: float = 2.0, daily_vol_spike_multiplier: float = 2.0,
                     check_schema: bool = False, profile: bool = False, profile_lines: bool = False,
                     use_delta: bool = False, use_raw_macro: bool = False, raw_fred_csv: str = None,
                     raw_vix_json: str = None, raw_dxy_csv: str = None, raw_news_dir: str = None):
    ...
    try:
        ...
        try:
            ...
            if not dry_run:
                ...
                if result.returncode == 0:
                    ...
                    results['macro_data'] = {'status': 'success', 'output': result.stdout}
                else:
        ...
        if not dry_run:
            ...
            try:
                ...
                if success_count == total_count:
                    ...
                    print(f"⚠️ {success_count}/{total_count} tables registered successfully")
            ...
            if use_delta:
                ...
                try:
                    ...
                    delta_tables = [
                        ('staged/stocks_daily_clean.parquet', f's3a://{s3_bucket}/{s3_prefix}stocks_daily.delta'),
                        ('staged/options_daily_clean.parquet', f's3a://{s3_bucket}/{s3_prefix}options_daily.delta'),
                        ('staged/stocks_30min_clean.parquet', f's3a://{s3_bucket}/{s3_prefix}stocks_30min.delta'),
                        ('staged/options_30min_clean.parquet', f's3a://{s3_bucket}/{s3_prefix}options_30min.delta'),
                        ('datasets/intraday_master.parquet', f's3a://{s3_bucket}/{s3_prefix}intraday_master.delta')
                    ]
                    ...
                    for parquet_path, delta_path in delta_tables:
                        if os.path.exists(parquet_path):
                            ...
                            success = write_delta_table(
                                df, delta_path, 
                                partition_cols=['year', 'month'] if 'year' in df.columns else None,
                                merge_schema=True
                            )
                ...
                results['delta_tables'] = {'status': 'skipped', 'reason': 'not_requested'}
        ...
        if not dry_run:
            ...
            try:
                ...
                passed_count = sum(1 for r in validation_results.values() if r.get("success", False))
                total_count = len(validation_results)
                
                if passed_count == total_count:
                    print(f"✅ All {total_count} datasets passed data quality validation")
                else:
                    print(f"⚠️ {passed_count}/{total_count} datasets passed validation")
                    
                    # Send Slack alert for validation failures
                    failed_datasets = [k for k, v in validation_results.items() if not v.get("success", False)]
                    if failed_datasets:
                        try:
            ...
            results['feast_materialization'] = {'status': 'skipped', 'reason': 'dry_run'}
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run full options pipeline with advanced signals")
    ...
    parser.add_argument("--check-schema", action="store_true", help="Validate Parquet schemas against specification")
    parser.add_argument("--profile", action="store_true", help="Enable performance profiling")
    parser.add_argument("--profile-lines", action="store_true", help="Enable line-by-line profiling with kernprof")
    parser.add_argument("--use-delta", action="store_true", help="Write outputs as Delta tables instead of Parquet")
    ...
    if args.profile_lines:
        ...
        cmd.extend(["--daily-vol-spike-multiplier", str(args.daily_vol_spike_multiplier)])
