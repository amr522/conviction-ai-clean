...
def check_data_drift(reference_path: str, current_path: str, 
                    drift_enabled: bool = False, drift_threshold: float = 0.1,
                    json_output: str = None) -> dict:
    ...
    numeric_cols = reference_data[common_cols].select_dtypes(include=['number']).columns.tolist()
    ...
    if drift_enabled and overall_drift:
        print(f"❌ Data drift detected (max score: {max_drift_score:.4f} > {drift_threshold})")
        sys.exit(1)
    elif drift_enabled:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Check for data drift between datasets")
    ...
    parser.add_argument("--drift-enabled", action="store_true", help="Enable drift detection (fail on drift)")
    parser.add_argument("--drift-threshold", type=float, default=0.1, help="Drift threshold (default: 0.1)")
