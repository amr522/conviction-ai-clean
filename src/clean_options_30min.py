...
from utils.performance_utils import (PERF_CONFIG,
...
def run(
    ...
    try:
        ...
        raw_df = (
            pl.scan_parquet(input_path, schema=None, extra_columns="ignore")
            .limit(50000)
            .collect()
        )
        ...
        try:
            ...
            df_pandas["timestamp"] = pd.to_datetime(
                df_pandas["window_start"], unit="ns"
            )
            ...
            df_pandas = df_pandas[
                (df_pandas["timestamp"] >= market_open)
                & (df_pandas["timestamp"] < market_close)
            ]
            ...
            df_pandas["transactions"] = (
                df_pandas["transactions"].fillna(0).astype("UInt32")
            )
        ...
        print(
            f"Applying strike grid selection ({strikes_below} below, {strikes_above} above ATM)..."
        )
        filtered_rows = []
        ...
        for underlying in df_pandas["underlying"].unique():
            ...
            print(
                f"{underlying}: ATM ~${underlying_price:.0f}, grid {len(strike_grid)} strikes (${min(strike_grid):.0f}-${max(strike_grid):.0f})"
            )
        ...
        cleaned = (
            ...
            .with_columns(
                [
                    ...
                    ((pl.col("opt30_high") + pl.col("opt30_low")) / 2).alias(
                        "opt30_mid_price"
                    ),
                    (
                        (pl.col("opt30_high") - pl.col("opt30_low"))
                        / pl.col("opt30_close")
                    ).alias("opt30_bid_ask_spread"),
                    # Basic Greeks estimates
                    (pl.col("opt30_close") / 100.0)
                    .clip(0.1, 2.0)
                    .alias("opt30_implied_volatility"),
            ...
            .with_columns(
                [
                    ...
                    (pl.col("opt30_vol_ratio") > vol_spike_multiplier).alias(
                        "opt30_vol_spike"
                    )
                ]
            )
        ...
        cleaned = compute_gamma_signals_optimized(
            cleaned,
            window=window_size,
        ...
        cleaned = cleaned.with_columns(
            ...
            + [
                pl.col("opt30_vol_spike").cast(pl.Boolean)
                if "opt30_vol_spike" in cleaned.columns
                else pl.lit(None)
            ]
        )
        ...
        if "opt30_gamma_squeeze" in cleaned.columns:
            ...
            print(
                f"Gamma squeezes detected: {squeeze_count}/{cleaned.shape[0]} ({squeeze_pct:.1f}%)"
            )
        print(f"Unique strikes processed: {unique_strikes}")
        print(
            f"Aggregation ratio: {data.shape[0] / cleaned.shape[0]:.2f} minutes per bar"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help="Minimum volume threshold (default: 10)",
    )
    parser.add_argument(
        "--strikes-below",
        type=int,
        default=5,
        help="Number of strikes below ATM (default: 5)",
    )
    parser.add_argument(
        "--strikes-above",
        type=int,
        default=5,
        help="Number of strikes above ATM (default: 5)",
    )
    parser.add_argument(
    ...
    result = run(
        ...
        vol_spike_multiplier=args.vol_spike_multiplier,
