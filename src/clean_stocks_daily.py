...
def run(date: str, dry_run: bool = False) -> dict:
    ...
    try:
        ...
        for i, file_path in enumerate(parquet_files):
            print(f"Processing {os.path.basename(file_path)} ({i+1}/{len(parquet_files)})...")
        ...
        try:
            ...
            if 'window_start' in df_pandas.columns:
                ...
                df_pandas['window_start'] = pd.to_datetime(df_pandas['window_start'], unit='ns', errors='coerce')
        ...
        print(f"Filtered daily stock data: {filtered_count}/{total_raw} rows for {date}")
