...
def generate_training_dataset(
    feature_path: str, label_path: str, output_path: str
) -> None:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generate training dataset from features and labels"
    )
    parser.add_argument(
        "--feature-path", required=True, help="Path to features parquet file"
    )
    parser.add_argument(
        "--label-path", required=True, help="Path to labels parquet file"
    )
    parser.add_argument(
        "--output-path",
        default="data/Parquet_data/train_dataset_{date}.parquet",
