...
from utils.performance_utils import optimize_join_performance, PERF_CONFIG
...
def run(date: str, dry_run: bool = False) -> dict:
    ...
    try:
        ...
        df = optimize_join_performance(stocks_df, options_df, on=["timestamp", "ticker"])
        
        # Native Polars dtype enforcement (no pandas conversion needed)
        float_cols = ['stock30_close_return', 'stock30_rolling_vol_5', 'opt30_strike', 'opt30_moneyness',
                     'opt30_mid_price_return', 'opt30_bid_ask_spread', 'opt30_implied_volatility', 'opt30_delta',
                     'opt30_theta', 'opt30_volume_return', 'opt30_rolling_vol_5', 'opt30_call_flow', 'opt30_put_flow',
                     'opt30_flow_divergence', 'opt30_net_gamma', 'opt30_gamma_mean_5', 'opt30_gamma_std_5']
        ...
        if 'opt30_flow_divergence' in df.columns:
            flow_non_null = df.filter(pl.col('opt30_flow_divergence').is_not_null()).shape[0]
            print(f"Flow divergence coverage: {flow_non_null}/{joined_count} ({(flow_non_null/joined_count)*100:.1f}%)")
        
        if 'opt30_gamma_squeeze' in df.columns:
            squeeze_non_null = df.filter(pl.col('opt30_gamma_squeeze').is_not_null()).shape[0]
            squeeze_true = df.filter(pl.col('opt30_gamma_squeeze') == True).shape[0]
            print(f"Gamma squeeze coverage: {squeeze_non_null}/{joined_count} ({(squeeze_non_null/joined_count)*100:.1f}%)")
            print(f"Gamma squeeze signals: {squeeze_true}/{joined_count} ({(squeeze_true/joined_count)*100:.1f}%)")
