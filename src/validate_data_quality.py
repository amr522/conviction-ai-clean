...
def create_expectation_suite(
    context: gx.DataContext, suite_name: str, dataset_type: str
) -> None:
    """
    ...
    if dataset_type == "options_daily":
        expectations = [
            ...
            {
                "expectation_type": "expect_column_to_exist",
                "kwargs": {"column": "optd_volume"},
            },
            # Data types
            {
                "expectation_type": "expect_column_values_to_be_of_type",
                "kwargs": {"column": "optd_close", "type_": "float64"},
            },
            {
                "expectation_type": "expect_column_values_to_be_of_type",
                "kwargs": {"column": "optd_volume", "type_": "int64"},
            },
            # Null checks
            {
        ...
        expectations = [
            ...
            {
                "expectation_type": "expect_column_to_exist",
                "kwargs": {"column": "opt30_close"},
            },
            {
                "expectation_type": "expect_column_to_exist",
                "kwargs": {"column": "opt30_volume"},
            },
            # Data types
            {
                "expectation_type": "expect_column_values_to_be_of_type",
                "kwargs": {"column": "opt30_close", "type_": "float64"},
            },
            {
                "expectation_type": "expect_column_values_to_be_of_type",
                "kwargs": {"column": "opt30_volume", "type_": "int64"},
            },
            # Null checks
            {
                "expectation_type": "expect_column_values_to_not_be_null",
    ...
    try:
        ...
        try:
            ...
            datasource = context.sources.add_pandas_filesystem(
                name=datasource_name, base_directory=str(Path(file_path).parent)
            )
        ...
        if not success:
            ...
            validation_result = results["run_results"][
                list(results["run_results"].keys())[0]
            ]
            failed_expectations = []
                    ...
                    failed_expectations.append(
                        {
                            ...
                            "observed_value": result["result"].get(
                                "observed_value", "N/A"
                            ),
                        }
                    )
            ...
            for failure in failed_expectations[:5]:  # Show first 5 failures
                logger.error(
                    f"  - {failure['expectation_type']} on {failure['column']}: {failure['observed_value']}"
                )
...
def generate_validation_report(
    validation_results: Dict, output_dir: str = "metrics"
) -> str:
    """
    ...
    try:
        ...
        report_path = os.path.join(
            output_dir, f"ge_validation_{dataset_type}_{date}.html"
        )
...
def validate_pipeline_outputs(
    date: str, output_dir: str = "staged", report_dir: str = "metrics"
) -> Dict[str, bool]:
    """
        ...
        if os.path.exists(file_path):
            try:
                validation_result = validate_dataset(
                    file_path, dataset_type, date, context
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output-dir", default="staged", help="Directory containing output files"
    )
    parser.add_argument(
        "--report-dir", default="metrics", help="Directory to save reports"
    )
