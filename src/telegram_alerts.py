...
class TelegramAlerter:
    ...
    def drift_alert(
        self, drift_score: float, threshold: float, features: List[str]
    ) -> bool:
...
"""
    ...
    def explainability_alert(
        self, feature_changes: Dict[str, float], threshold: float
    ) -> bool:
        """Send model explainability alert."""
        max_change = max(feature_changes.values())
        top_changes = sorted(
            feature_changes.items(), key=lambda x: abs(x[1]), reverse=True
        )[:5]

        change_text = "\n".join(
            [f"• `{feat}`: {change:+.3f}" for feat, change in top_changes]
        )
...
def check_and_alert_drift(drift_report_path: str, config: AlertConfig) -> bool:
    ...
    try:
        ...
        if max_drift > config.drift_threshold:
            ...
            return alerter.drift_alert(
                max_drift, config.drift_threshold, drifted_features
            )
...
def check_and_alert_signals(
    signal_metrics: Dict[str, float], config: AlertConfig
) -> bool:
...
def check_and_alert_explainability(
    shap_changes: Dict[str, float], config: AlertConfig
) -> bool:
    """Check SHAP importance changes and send alert if needed."""
    max_change = (
        max(abs(change) for change in shap_changes.values()) if shap_changes else 0.0
    )

    if max_change > config.explainability_threshold:
        alerter = TelegramAlerter(config.bot_token, config.chat_id)
        return alerter.explainability_alert(
            shap_changes, config.explainability_threshold
        )
...
def main():
    ...
    parser.add_argument(
        "--type", choices=["drift", "signal", "explainability"], required=True
    )
