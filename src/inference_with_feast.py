...
from feast_materialize import get_feature_store, get_online_features
...
def get_inference_features(
    tickers: List[str], feature_views: List[str] = None, repo_path: str = "feature_repo"
    ...
    try:
        ...
        features_dict = get_online_features(
            entity_rows=entity_rows, feature_names=feature_names, repo_path=repo_path
        )
        ...
        logger.info(
            f"Retrieved features for {len(tickers)} tickers: {features_df.shape}"
        )
        return features_df
...
def batch_inference(
    ...
    try:
        ...
        output_df = pd.DataFrame(
            {"ticker": results["tickers"], "prediction": results["predictions"]}
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--tickers", nargs="+", help="Ticker symbols for single inference"
    )
