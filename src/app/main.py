...
def get_features_from_store(ticker: str, timestamp: Optional[str] = None) -> Dict:
    ...
    with start_span(op="feature_fetch", description="Fetch features from Feast store") as span:
        try:
            # Set span data
...
@app.post("/predict", response_model=InferenceResponse, dependencies=[Depends(verify_predict_permission)])
...
async def predict(request: Request, inference_request: InferenceRequest, token_data: TokenData = Depends(verify_predict_permission)):
    ...
    try:
        ...
        if inference_request.features:
            ...
            try:
                features = get_features_from_store(inference_request.ticker, inference_request.timestamp)
...
@app.post("/predict/batch", response_model=BatchInferenceResponse, dependencies=[Depends(verify_batch_permission)])
...
async def predict_batch(request: Request, batch_request: BatchInferenceRequest, token_data: TokenData = Depends(verify_batch_permission)):
