...
FEATURE_STORE_REQUESTS = Counter(
    "feature_store_requests_total", "Total requests to feature store", ["status"]
)
    ...
    async def wrapper(*args, **kwargs):
        ...
        try:
            ...
            ERROR_COUNTER.labels(
                error_type=type(e).__name__, endpoint="/predict/batch"
            ).inc()
            raise
...
async def metrics_middleware(request: Request, call_next):
    ...
    try:
        ...
        # Track request
        PREDICTIONS_TOTAL.labels(
            method=method, endpoint=path, status_code=status_code, user_id=user_id
        ).inc()
        ...
        # Track error
        ERROR_COUNTER.labels(error_type=type(e).__name__, endpoint=path).inc()
        raise
        ...
        duration = time.time() - start_time
        PREDICTION_LATENCY.labels(
            method=method, endpoint=path, model_version="unknown"
        ).observe(duration)
