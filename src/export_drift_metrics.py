...
def export_drift_metrics(json_path: str, pushgateway_url: str, job: str = "data_drift"):
    ...
    max_drift_gauge = Gauge(
        "data_drift_max_score", "Maximum feature drift score", registry=registry
    )
    ...
    threshold_gauge = Gauge(
        "data_drift_threshold", "Configured drift threshold", registry=registry
    )
    features_analyzed_gauge = Gauge(
        "data_drift_features_analyzed", "Number of features analyzed", registry=registry
    )
